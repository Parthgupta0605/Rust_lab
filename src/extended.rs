...
use std::env;
use std::fs::File;
use std::io::{self, stdout, BufReader, BufWriter, Result, Write};
use std::path::Path;
...
impl CellAddress {
    ...
    fn to_string(&self) -> String {
        format!("{}{}", ((self.col as u8) + b'A') as char, self.row + 1)
    }
}
...
impl Spreadsheet {
    ...
    fn move_cursor(&mut self, dx: isize, dy: isize) {
        ...
        if new_col >= 0
            && new_col < self.max_cols as isize
            && new_row >= 0
            && new_row < self.max_rows as isize
        {
        ...
        if let Some(old_cell) = self.get_cell(addr).cloned() {
            ...
            if let Some(mut cell) = cell_clone {
                ...
                if value.starts_with("=") {
                    // Validate formula
                    let formula = &value[1..];
                    let is_valid_formula = if formula.starts_with("SUM(")
                        || formula.starts_with("MIN(")
                        || formula.starts_with("MAX(")
                        || formula.starts_with("STDEV(")
                    {
                        if let Some(range_str) = formula
                            .strip_prefix("SUM(")
                            .or_else(|| formula.strip_prefix("MIN("))
                            .or_else(|| formula.strip_prefix("MAX("))
                            .or_else(|| formula.strip_prefix("STDEV("))
                        ...
                        if let Some(arg) = formula
                            .strip_prefix("sqrt(")
                            .or_else(|| formula.strip_prefix("log("))
                            .and_then(|s| s.strip_suffix(')'))
                        {
                            CellAddress::from_str(arg)
                                .map_or(false, |addr| self.get_cell(&addr).is_some())
                                || arg.parse::<f64>().is_ok()
                        } else {
                            false
                        }
    ...
    fn process_command(&mut self) -> bool {
        ...
        if cmd == "q" {
            ...
            if parts[0] != "i" {
                self.status_message =
                    "INVALID COMMAND - Do you mean to write :i (cell name)".to_string();
            }
            if parts.len() > 1 {
    ...
    fn draw(&self, stdout: &mut io::Stdout) -> io::Result<()> {
        ...
        if !status_message.is_empty() {
            stdout.execute(cursor::MoveTo(
                cols.saturating_sub(status_message.len() as u16),
...
pub fn run_extended() -> Result<()> {
    ...
    let (rows, cols) = if args.len() >= 4 {
        // Changed from 3 to 4 to account for the -vim flag
        let r = args[2].parse::<usize>().unwrap_or(10);
        let c = args[3].parse::<usize>().unwrap_or(10);
        (r, c)
    } else {
        eprintln!(
            "Usage: {} -vim <rows> <cols>. Defaulting to 10x10.",
            args[0]
        );
        (10, 10)
    };
